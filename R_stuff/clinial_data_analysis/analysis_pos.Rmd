...
na_count_bl_pos <- as.data.frame( sapply(df_meta_bl[-1], function(y) sum(length(which(is.na(y))))) )
na_count_y2_pos <- as.data.frame( sapply(df_meta_y2[-1], function(y) sum(length(which(is.na(y))))) )
names(na_count_bl_pos) = "na_count_bl_pos"
names(na_count_y2_pos) = "na_count_y2_pos"
na_count_bl_pos = na_count_bl_pos %>% 
  dplyr::add_rownames( var ="CTSC_local_Lab")
na_count_y2_pos = na_count_y2_pos %>% 
  dplyr::add_rownames( var ="CTSC_local_Lab")

readr::write_csv( na_count_bl_pos, "outputs/na_count_bl_pos.csv")
readr::write_csv( na_count_y2_pos, "outputs/na_count_y2_pos.csv")
...
df_all_bl_spearman= df_all_bl %>% 
  tidyr::gather( `3231728`: `7198670`, key = "meta", value = "meta_reading") %>% 
  tidyr::nest( -meta) %>% 
...
df_all_delta_pearson$delta_pearson_cor
...
readr::write_csv(df_final_report, "outputs/ctsc_report_pos.csv" )
```
